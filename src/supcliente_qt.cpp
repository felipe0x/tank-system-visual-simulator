  ...
  , image(new SupImg(this))
{
  ...
  //ADICIONADO
  connect(this, &SupClienteQt::signStateStored,
          this, &SupClienteQt::slotUpdateState);
...
//ADICIONADO
void SupClienteQt::slotUpdateState()
{
  image->addPoint(deltaT(), lastState());
}
...
void SupClienteQt::slotConectar(QString IP, QString Login, QString Senha)
{
  ...
  /* ACRESCENTAR */
  std::string ip = IP.toStdString();
  std::string login = Login.toStdString();
  std::string senha = Senha.toStdString();
...
void SupClienteQt::slotExibirInterface()
{
  ...
  if (estahConectado)
  {
    ...
    const SupState& State = lastState();

    showValves(State.V1, State.V2);
    showPump(State.PumpInput);
    showH(State.H1, State.H2, State.ovfl);
    showFlow(State.PumpFlow);
